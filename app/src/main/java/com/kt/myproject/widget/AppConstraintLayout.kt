...
open class AppConstraintLayout : ConstraintLayout {
    ...
    private fun render(attrs: AttributeSet?) {
        ...
        val radius = a.pixels(R.styleable.AppConstraintLayout_radius)

        val attrTopLeftRadius = a.pixels(R.styleable.AppConstraintLayout_radiusTopLeft)
        radiusTopLeft = if (attrTopLeftRadius > 0F) attrTopLeftRadius else radius

        val attrTopRightRadius = a.pixels(R.styleable.AppConstraintLayout_radiusTopRight)
        radiusTopRight = if (attrTopLeftRadius > 0F) attrTopRightRadius else radius

        val attrBottomLeftRadius = a.pixels(R.styleable.AppConstraintLayout_radiusBottomLeft)
        radiusBottomLeft = if (attrTopLeftRadius > 0F) attrBottomLeftRadius else radius

        val attrBottomRightRadius = a.pixels(R.styleable.AppConstraintLayout_radiusBottomRight)
        radiusBottomRight = if (attrTopLeftRadius > 0F) attrBottomRightRadius else radius

        backgroundColor = a.getColor(R.styleable.AppConstraintLayout_backgroundColor, Color.WHITE)

        strokeLineWidth = a.pixels(R.styleable.AppConstraintLayout_strokeLineWidth)
        strokeLineColor = a.getColor(R.styleable.AppConstraintLayout_strokeLineColor, Color.BLACK)

        dashLineWidth = a.pixels(R.styleable.AppConstraintLayout_dashLineWidth)
        dashLineGap = a.pixels(R.styleable.AppConstraintLayout_dashLineGap)
