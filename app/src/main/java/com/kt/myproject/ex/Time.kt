...
private val serverDataFmt by lazy {
    SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'Z'", Locale.getDefault()).also {
        it.timeZone = TimeZone.getTimeZone("GMT")
    }
}

val hmdmyFmt by lazy {
    SimpleDateFormat("HH:mm - dd/MM/yyyy")
}

fun String?.getDateWithServerTimeStamp(): String? {
    return try {
        serverDataFmt.parse(this).dateFormat(hmdmyFmt)
